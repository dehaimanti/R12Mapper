...
def ask_llm_for_mappings(headers, user_table_map, user_column_map, user_comment_map, metadata_df=None):
    ...
    system_prompt_step1 = (
        "You are an Oracle R12 expert. For each label, guess the Oracle R12 TABLE name "
        "based on label text and hints. Use _All or _B tables whereever applicable. Do NOT guess columns. Format:\n"
    ...
    try:
        content1 = re.sub(r"//.*", "", content1)  # remove inline comments
    ...
    system_prompt_step2 = (
        "You are an Oracle R12 expert. Using the metadata and hints, map each label to the correct Oracle R12 TABLE and COLUMN.Use _All or _B tables whereever applicable.\n"
        "The column names could be exact names and could also be not exact names. Let llm decide. Return ONLY a JSON array like this:\n"
    ...
    try:
        raw_content = re.sub(r"//.*", "", raw_content)  # 🧹 Remove comments
    ...
    for item in llm_mappings:
        ...
        if not found_match:
            ...
            print(f"⚠️ Table '{llm_table}' not found, look for fallback '{alt_table}'.")
        ...
        if found_match:
            print(f"Found Match for '{llm_column}'.")
